...
class PatternMatcherApp:
    ...
    def engine_rk(self, M, P):
        """
        2D Rabin–Karp with rolling hash (horizontal + vertical).
        - Precompute rolling hashes for every length-PC window in each matrix row.
        - Combine PR consecutive row-window hashes vertically using a second rolling hash.
        - Verify candidates on hash match to guarantee correctness.
        """
        NR, NC, PR, PC = len(M), len(M[0]), len(P), len(P[0])
        steps, comps, matches = [], 0, 0

        # Two-mod hashing to reduce collision risk
        ...
        def row_window_hashes(row, win, base, mod):
            """Rolling hash for all contiguous windows of length win in a 1D integer row."""
            if win > len(row):
                return []
        ...
        def build_2d_hash_grid(M_):
            """Returns vhash[i][j] = (h1,h2) hash of PRxPC submatrix at top-left (i,j)."""
            # 1) Horizontal rolling hashes for each row
            row_h1 = [row_window_hashes(M_[r], PC, base_col1, mod1) for r in range(NR)]
            row_h2 = [row_window_hashes(M_[r], PC, base_col2, mod2) for r in range(NR)]
            ...
            # 2) Vertical rolling hash per column window-hash index j
        ...
        for i in range(out_rows):
            for j in range(out_cols):
                ...
                is_match = False
                if vhash[i][j] == p_hash:
                    # Verify to guarantee correctness
                    is_match = True
                    cells = []
                    for pi in range(PR):
                        for pj in range(PC):
                    ...
                    # Minimal cell info for visualization, consistent with other engines
