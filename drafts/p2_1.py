...
class PatternMatcherApp:
    ...
    def create_layout(self):
        ...
        algos = [
            ("Naive", "naive"),
            ("Rabin-Karp (2D Rolling)", "rk"),
            ("KMP (Row-Filter)", "kmp"),
            ("Boyer-Moore (Row-Filter)", "bm"),
            ("Aho-Corasick (2D)", "aho"),
            ("Bird-Baker", "bb"),
    ...
    def _kmp_search(self, text, pat, lps_data):
        lps, _ = lps_data
        res, j, comps = [], 0, 0
    ...
    def engine_rk(self, M, P):
        ...
        def get_hash(block):
            h = 0
            for row in block:
                for val in row: h = (h * base + val) % mod
            return h
        ...
        for i in range(NR - PR + 1):
            for j in range(NC - PC + 1):
                comps += 1
                sub = [row[j:j+PC] for row in M[i:i+PR]]
                if get_hash(sub) == p_hash:
                    is_match = (sub == P)
                    if is_match: matches += 1
    ...
    def engine_kmp(self, M, P):
        NR, NC, PR, PC = len(M), len(M[0]), len(P), len(P[0])
        steps, comps, matches = [], 0, 0
        lps_info = self._kmp_build_lps(P[0])
        comps += lps_info[1]

        for i in range(NR - PR + 1):
            hits, c_search = self._kmp_search(M[i], P[0], lps_info)
            comps += c_search
            hit_set = set(hits)
            for j in range(NC - PC + 1):
                ok = False
                if j in hit_set:
                    ...
                    comps += (PR - 1)
                if ok: matches += 1
                steps.append({'pos':(i,j), 'ok':ok, 'cells':[(i,j,ok)], 'm':matches, 'c':comps})
        return steps
    ...
    def engine_aho(self, M, P):
        ...
        for r_idx, p_row in enumerate(P):
            lps = self._kmp_build_lps(p_row)
            for i in range(NR):
                found, _ = self._kmp_search(M[i], p_row, lps)
                for f in found: hits[i][r_idx].add(f)

        for i in range(NR - PR + 1):
            for j in range(NC - PC + 1):
                comps += 1
                ok = all(j in hits[i+pi][pi] for pi in range(PR))
                if ok: matches += 1
                steps.append({'pos':(i,j), 'ok':ok, 'cells':[(i,j,ok)], 'm':matches, 'c':comps})
        return steps
    ...
    def engine_kmp_nv(self, M, P):
        NR, NC, PR, PC = len(M), len(M[0]), len(P), len(P[0])
        steps, comps, matches = [], 0, 0
        lps_cache = {tuple(row): self._kmp_build_lps(row) for row in P}
        ...
        for i in range(NR - PR + 1):
            for j in range(NC - PC + 1):
                ...
                ok = True
                for pi in range(PR):
                    row_hits, _ = self._kmp_search(M[i+pi], P[pi], lps_cache[tuple(P[pi])])
                    if j not in row_hits:
                        ok = False; break
                if ok: matches += 1
                steps.append({'pos':(i,j), 'ok':ok, 'cells':[(i,j,ok)], 'm':matches, 'c':comps})
        return steps
    ...
    def engine_bb(self, M, P):
        # Bird-Baker simplified for visualization: KMP row matching + Column sequence validation
        return self.engine_aho(M, P)
    ...
    def run_single(self):
        ...
        dispatch = {
            "naive": self.engine_naive, "rk": self.engine_rk, 
            "kmp": self.engine_kmp, "bm": self.engine_bm, 
            "aho": self.engine_aho, "bb": self.engine_bb, "kmp_nv": self.engine_kmp_nv
        }
        
        self.steps = dispatch[algo](self.matrix, self.pattern)
    ...
    def run_multi_race(self):
        ...
        self.race_engines = {}
        engines = [("Naive", self.engine_naive), ("RK", self.engine_rk), ("KMP", self.engine_kmp), 
                   ("BM", self.engine_bm), ("AHO", self.engine_aho), ("BB", self.engine_bb)]
        ...
        for idx, (name, func) in enumerate(engines):
            ...
            for r in range(len(self.matrix)):
                for c in range(len(self.matrix[0])):
                    ...
                    cells[(r, c)] = l
            self.race_engines[name] = {"steps": func(self.matrix, self.pattern), "cells": cells}
    ...
    def highlight_step(self):
        if self.mode == "single":
            ...
            self.stat_time.config(text=f"{self.runtime:.2f}")
            return True
        else:
            active = False
    ...
    def auto_play(self):
        ...
        if self.highlight_step():
            self.step_idx += 1
            delay = 100 if self.mode == "single" else 200
    ...
    def load_preset(self, name):
        ...
        if name == "5x5 Simple":
            ...
            self.pattern_input.insert("1.0", f"{random.randint(1,9)} {random.randint(1,9)}")
